...
contract StrategycrLINK {
    ...
    address public constant crLINK = address(
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            //Approve crLINK pool to spend idle YFI
            IERC20(want).safeApprove(crLINK, 0);
            IERC20(want).safeApprove(crLINK, _want);
            //Mint _want amount of crLINK interest bearing tokens.
            cToken(crLINK).mint(_want);
        }
    }
    ...
    function withdraw(IERC20 _asset) external returns (uint256 balance) {
        ...
        require(crLINK != address(_asset), "crLINK");
    ...
    function balanceCInToken() public view returns (uint256) {
        ...
        uint256 b = balanceC();
        if (b > 0) {
            b = b.mul(cToken(crLINK).exchangeRateStored()).div(1e18);
        }
        return b;
    }

    function balanceC() public view returns (uint256) {
        return IERC20(crLINK).balanceOf(address(this));
    }
