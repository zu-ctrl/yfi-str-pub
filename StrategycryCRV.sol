...
contract StrategycryCRV {
    ...
    address public constant cryCRV = address(
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            //Approve cryCRV pool to spend idle YFI
            IERC20(want).safeApprove(cryCRV, 0);
            IERC20(want).safeApprove(cryCRV, _want);
            //Mint _want amount of cryCRV interest bearing tokens.
            cToken(cryCRV).mint(_want);
        }
    }
    ...
    function withdraw(IERC20 _asset) external returns (uint256 balance) {
        ...
        require(cryCRV != address(_asset), "cryCRV");
    ...
    function balanceCInToken() public view returns (uint256) {
        ...
        uint256 b = balanceC();
        if (b > 0) {
            b = b.mul(cToken(cryCRV).exchangeRateStored()).div(1e18);
        }
        return b;
    }

    function balanceC() public view returns (uint256) {
        return IERC20(cryCRV).balanceOf(address(this));
    }
